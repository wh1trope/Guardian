...
/**
 * Validates command block minecart set command packets to prevent privilege escalation.
 */
public class CommandMinecartProcessor implements PacketProcessor {

    private final GuardianModule module;
    private long commandOffset = -1;
    private boolean fieldsMapped = false;

    public CommandMinecartProcessor(GuardianModule module) {
        this.module = module;
    }
    ...
    public boolean process(Object packet, Player player, String packetName, Channel channel) {
        ...
        if (!player.hasPermission("minecraft.command.commandblock") && !player.isOp()) {
            module.flag(player, "Exploit: SetCommandMinecart without permission", 10.0);
            return false;
        }

        if (commandOffset != -1) {
            Object commandObj = UnsafeUtil.getObject(packet, commandOffset);
            if (commandObj instanceof String command) {
                if (command.length() > 32767) {
                    module.flag(player, "Exploit: Oversized CommandMinecart String", 10.0);
                    return false;
                }
            }
        }
    ...
    private void mapFields(Class<?> clazz) {
        for (Field f : ReflectionUtil.getCachedFields(clazz)) {
            if (f.getType() == String.class) {
                commandOffset = UnsafeUtil.objectFieldOffset(f);
                break;
            }
        }
        fieldsMapped = true;
